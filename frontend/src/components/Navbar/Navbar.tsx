...
const Navbar: FC = () => {
  ...
  const onNavbarLinkClick = () => {
    if (checkbox && checkbox.current && window.innerWidth < 1280) checkbox.current.checked = !checkbox.current.checked;
  };
  ...
  return (
    <header className={styles.header}>
      ...
      <ul className={styles.menu}>
        ...
        {pages.map((x) => (
          <li onClick={onNavbarLinkClick} key={x.path}>
        ...
        {isAuth ? (
          <>
            <li onClick={onNavbarLinkClick}>
              <Link to="/account">
                <UserOutlined style={userIcon} /> {me?.username}
              </Link>
            </li>
            <li onClick={onNavbarLinkClick}>
              <a onClick={signOut}>
                <LogoutOutlined style={userIcon} /> Sign Out
              </a>
            </li>
          </>
        ) : (
          <>
            <li onClick={onNavbarLinkClick}>
              <Link to="/sign-in">
                <LoginOutlined style={userIcon} />
                Sign In
              </Link>
            </li>
            <li onClick={onNavbarLinkClick}>
              <Link to="/sign-up">
                <PlusCircleOutlined style={userIcon} />
                Sign Up
              </Link>
            </li>
          </>
